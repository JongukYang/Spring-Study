...
class Solution {
    private static class Device {
        ...
        public Device(int location, int delay) {
    ...
    public static void main(String args[]) throws Exception {
		...
		*/
        ...
        for (int test_case = 1; test_case <= T; test_case++) {
            ...
            for (int i = 0; i < N; i++) {
                for (int j = 0; j < 2; j++) {
                    devices[i][j] = sc.nextInt();
                }
            }

            int left = 0; // 왼쪽에서 발사
            int right = D; // 오른쪽에서 발사
            int time = 0; // 전체 시간
            // devices 배열의 idx
            int idx = 0;
            ...
            while (true) {
                ...
                // 왼쪽에서 발사한 폭죽이 장치를 만나면
                if (devices[idx][0] - left <= right - devices[idx][0]) {
                    for (int i = 0; i < devices[idx][1]; i++) {
                        right--;
                        time++;
                    }
                    idx++;
